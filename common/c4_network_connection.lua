...
function NetworkConnectionBase:SendCommand(sCommand, ...)
	if(self._IsConnected) then
		if(self._IsOnline) then
			...
			print("Sending Command from NetworkConnectionBase:SendCommand")

			print("Command Info:")
			print("Binding: " .. self._BindingID .. " Port: " .. self._Port .. " Command: " .. sCommand)
...
function NetworkConnectionBase:ReceivedFromNetwork(idBinding, nPort, sData)
	self._LastCheckin = 0
	print("RECEIVED_FROM_network_connection " .. sData)
